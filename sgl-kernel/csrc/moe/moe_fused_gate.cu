...
__global__ void moe_fused_gate_kernel(void* input, void* bias, float* output_ptr, int32_t* indices_ptr, int num_rows, int k) {
...
moe_fused_gate(at::Tensor& input, at::Tensor& bias) {
    ...
    moe_fused_gate_kernel<VPT, NUM_EXPERTS, THREADS_PER_ROW, ROWS_PER_WARP, ROWS_PER_CTA, WARPS_PER_CTA><<<num_blocks, block_dim, 0, stream>>>(
